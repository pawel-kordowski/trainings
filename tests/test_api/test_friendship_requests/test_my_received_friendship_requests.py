...
class TestMyReceivedFriendshipRequests:
    query = """
    {
    ...
    def test_successful(
        ...
        sender_by_friendship_request_id = {
            friendship_request_1.id: user_2,
            friendship_request_2.id: user_3,
        }
        mocked_friendship_request_service.get_pending_requests_received_by_user.return_value = (  # noqa
            friendship_requests
        )
        mocked_user_service.get_users_by_ids.return_value = [user_2, user_1, user_3]
        ...
        assert response.status_code == 200
        assert response.json()["data"]["myReceivedFriendshipRequests"] == [
            {
                ...
                "sender": {
                    "id": str(
                        sender_by_friendship_request_id[friendship_request.id].id
                    ),
                    "email": sender_by_friendship_request_id[
                        friendship_request.id
                    ].email,
                },
                "receiver": {"id": str(user_1.id), "email": user_1.email},
            }
            for friendship_request in friendship_requests
        ]
        mocked_friendship_request_service.get_pending_requests_received_by_user.assert_awaited_once_with(  # noqa
            user_id=user_1.id
        )
        mocked_user_service.get_users_by_ids.assert_awaited_once_with(
            [user_2.id, user_1.id, user_3.id]
        )
